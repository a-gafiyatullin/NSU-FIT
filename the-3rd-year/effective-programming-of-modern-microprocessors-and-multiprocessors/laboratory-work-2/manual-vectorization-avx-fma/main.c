	...
	for(int n = 0; n < Nt - 1; n++) {
		...
		for(int i = 1; i < Ny - 1; i++) {
			...
			for(int j = 0; j < Nx / VECTOR_SIZE; j++) {
				...
				__m256 rez = v_coeff1 * (
						_mm256_fmadd_ps(v_coeff3, (v_F_prev[j] + v_F_prev_shifted[j]),
						_mm256_fmadd_ps(v_coeff2b, (v_F_prev_vertical[j] + v_F_next_vertical[j]),
						_mm256_fmadd_ps(v_coeff2, (v_F_curr[j] + v_F_curr_shifted[j]),
						_mm256_fmadd_ps(v_coeff3, (v_F_next[j] + v_F_next_shifted[j]),

						_mm256_fmadd_ps(v_coeff5, v_p_curr_vertical[j],
