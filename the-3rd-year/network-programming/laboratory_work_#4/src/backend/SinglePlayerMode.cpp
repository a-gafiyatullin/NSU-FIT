...
SinglePlayerMode::SinglePlayerMode(const int32_t &width, const int32_t &height,
    ...
    : PlayerMode(width, height, food_static, food_per_player, state_delay_ms, 0) {
  local_player_snake = std::make_shared<Snake>(Point(width / 2, height / 2, width, height, 'o'), field);
}
...
std::shared_ptr<SinglePlayerMode> SinglePlayerMode::getInstance(
  ...
  if(instance == nullptr) {
    instance.reset(new SinglePlayerMode(width, height, food_static, food_per_player, state_delay_ms));
  }
