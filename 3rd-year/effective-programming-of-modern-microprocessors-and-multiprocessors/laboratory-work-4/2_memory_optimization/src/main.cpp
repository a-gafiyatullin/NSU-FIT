...
#pragma omp single
			...
			int starting_correction = (num_thread == 0 ? 0 : 1);	// correction for zero thread
			int finalizing_correction = (num_thread == THREADS - 1 ? 0 : 1);	// correction for last thread

			for (int k = 2 - 7 * starting_correction; k < 5 + 2 * (1 - starting_correction); k++) {
				compute_process(F0, F1, p, k + shift);
			}

			for (int k = 2 - 6 * starting_correction; k < 4 + 2 * (1 - starting_correction); k++) {
				compute_process(F1, F0, p, k + shift);
			}

			for(int k = 2 - 5 * starting_correction; k < 3 + 2 * (1 - starting_correction); k++) {
				compute_process(F0, F1, p, k + shift);
			}

			for (int k = 2 - 4 * starting_correction; k < 2 + 2 * (1 - starting_correction); k++) {
				compute_process(F1, F0, p, k + shift);
			}

			for (int k = 2 - 3 * starting_correction; k < 1 + 2 * (1 - starting_correction); k++) {
				compute_process(F0, F1, p, k + shift);
			}

			int last_element = shift + CHUNK;

			for (int i = shift + 7 - 2 * starting_correction; i < last_element - 3 - 2 * finalizing_correction; i++) {
...
#pragma omp barrier
			...
			for (int k = 0; k < 1 + finalizing_correction; k++) {
				compute_process(F1, F0, p, last_element - 4 - 2 * finalizing_correction + k);
			}

			for (int k = 0; k < 2 + 2 * finalizing_correction; k++) {
				compute_process(F0, F1, p, last_element - 5 - 2 * finalizing_correction + k);
			}

			for (int k = 0; k < 3 + 3 * finalizing_correction; k++) {
				compute_process(F1, F0, p, last_element - 6 - 2 * finalizing_correction + k);
			}

			for (int k = 0; k < 4 + 4 * finalizing_correction; k++) {
				compute_process(F0, F1, p, last_element - 7 - 2 * finalizing_correction + k);
			}

			for (int k = 0; k < 5 + 5 * finalizing_correction; k++) {
				compute_process(F1, F0, p, last_element - 8 - 2 * finalizing_correction + k);
			}
#pragma omp barrier
